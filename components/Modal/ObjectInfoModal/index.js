...
export default function ObjectInfoModal({ details, directionAction }) {
				...
				<>
					<div className="w-full">
						<Image
							...
							src={`https://gis-restoran.pockethost.io/api/files/restaurants/${details.id}/${details.image}`}
							alt="banner"
							width={1000}
							height={1000}
						/>
					...
					<div className="px-3 text-black">
						...
						{directionAction && (
							<button
								onClick={directionAction}
								className="mt-3 bg-red-500 rounded-md px-2 py-2 text-white text-xs font-bold hover:bg-red-700 hover:text-slate-200 transition-all duration-300"
							>
								Tunjukan Arah
							</button>
						)}
					...
					<div className="overflow-auto text-black px-5 h-full">
						{selectedMenu === "detail"
							? detailTab
							: selectedMenu === "galeri"
							? galeriTab
				...
				<>
					<div className="w-full h-full flex flex-col justify-center items-center text-black">
						<h1 className="font-semibold text-xl">
							<span className="font-extrabold leading-loose">
								SI
							</span>{" "}
							Geografis Restoran
						</h1>
						<span className="text-sm text-gray-700">
							(pilih marker untuk melihat detail)
						</span>
					</div>
				</>
			)}
